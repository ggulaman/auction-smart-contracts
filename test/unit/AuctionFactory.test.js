...
describe('AuctionFactoryUnit Test', function() { 
  ...
  it('only allows owner to raise auctions', async () => {
    await expect(auctionFactory.connect(signer0).createANewAuction(auctionDuration, ERC20Name, ERC20Symbol, ERC20Supply)).to.be.revertedWith("only owner");
  })
  ...
  it('allows owner to raise a new auction', async () => {
    ...
    await auctionFactory.connect(owner).createANewAuction(auctionDuration, ERC20Name, ERC20Symbol, ERC20Supply);
